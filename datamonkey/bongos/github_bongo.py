...
class GitHubBongo(BaseBongo):
    ...
    async def create_entities(self) -> List[Dict[str, Any]]:
        ...
        from datamonkey.generation.adapter_github import generate_github_artifact, slugify
        ...
        for i in range(self.entity_count):
            ...
            slug = slugify(title)[:40] or f"datamonkey-{token}"
    ...
    async def update_entities(self) -> List[Dict[str, Any]]:
        ...
        from datamonkey.generation.adapter_github import generate_github_artifact
        ...
        async with httpx.AsyncClient() as client:
            ...
            try:
                ...
                if check_response.status_code == 200:
                    ...
                    message = f"Update datamonkey test file: {filename}"
                    self.logger.info(f"   File exists, SHA: {current_sha}")
                else:
                    # File doesn't exist, create new
                    current_sha = None
                    message = f"Add datamonkey test file: {filename}"
    ...
    async def _delete_test_file(self, filename: str, sha: str):
        ...
        async with httpx.AsyncClient() as client:
            ...
            response = await client.request(
                ...
                data=json.dumps({
                    "message": f"Remove datamonkey test file: {filename}",
                    "sha": sha,
                    "branch": "main"
                })
            )
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    delete_response = await client.request(
                        ...
                        data=json.dumps({
                            "message": f"Force delete datamonkey test file: {filename}",
                            "sha": file_info["sha"],
                            "branch": "main"
                        })
                    )
    ...
    def _generate_updated_content(self, filename: str, file_num: int) -> str:
        ...
        if file_ext in ["md", "markdown"]:
            return f"""# Datamonkey Test File {file_num} - UPDATED
...
def datamonkey_test_function_{file_num}_updated():
    """Updated test function for datamonkey verification."""
    return f"Hello from UPDATED Datamonkey Test File {file_num}!"


class DatamonkeyTestClass{file_num}Updated:
    """Updated test class for datamonkey verification."""
    
    def __init__(self):
        self.name = f"Datamonkey Test Class {file_num} - UPDATED"
...
if __name__ == "__main__":
    print(datamonkey_test_function_{file_num}_updated())
    test_obj = DatamonkeyTestClass{file_num}Updated()
    print(test_obj.get_info())
'''
        elif file_ext == "json":
            return f'''{{
  "test_file": "datamonkey_test_{file_num}.json",
  "purpose": "Testing GitHub sync with UPDATED JSON content",
  "status": "UPDATED",
  "content": {{
    "title": "Datamonkey Test File {file_num} - UPDATED",
...
"""
    ...
    def _generate_file_content(self, file_type: str, file_num: int, random_suffix: str) -> str:
        ...
        if file_type in ["markdown", "md"]:
            return f"""# Datamonkey Test File {file_num}

This is test file {file_num} created by the datamonkey framework to verify GitHub sync functionality.
...
def datamonkey_test_function_{file_num}():
    """Test function for datamonkey verification."""
    return f"Hello from Datamonkey Test File {file_num}!"


class DatamonkeyTestClass{file_num}:
    """Test class for datamonkey verification."""
    
    def __init__(self):
        self.name = f"Datamonkey Test Class {file_num}"
...
if __name__ == "__main__":
    print(datamonkey_test_function_{file_num}())
    test_obj = DatamonkeyTestClass{file_num}()
    print(test_obj.get_info())
'''
        elif file_type == "json":
            return f'''{{
  ...
  "content": {{
    "title": "Datamonkey Test File {file_num}",
    ...
    "metadata": {{
      ...
      "timestamp": "{timestamp}"
            ...
            return f"""# Datamonkey Test File {file_num}
test_file: datamonkey_test_{file_num}.yml
