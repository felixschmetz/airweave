...
class AsanaBongo(BaseBongo):
    ...
    def __init__(self, credentials: Dict[str, Any], **kwargs):
        ...
        self.openai_model: str = kwargs.get("openai_model", "gpt-5")
    ...
    async def create_entities(self) -> List[Dict[str, Any]]:
        ...
        from datamonkey.generation.adapter_asana import generate_asana_task
        ...
        async with httpx.AsyncClient() as client:
            ...
            # Optionally allow concurrent creations
            results = await asyncio.gather(*(create_one() for _ in range(self.entity_count)))
    ...
    async def _ensure_project(self):
        if self._project_gid:
            return
        name = f"datamonkey-asana-test-{str(uuid.uuid4())[:6]}"
        async with httpx.AsyncClient() as client:
            r = await client.post(
