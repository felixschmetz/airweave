...
from datamonkey.backend.run_manager import RunManager


app = FastAPI(title="Datamonkey Backend", version="0.1.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
# Optional Redis for cross-process log streaming
...
async def ws_logs(ws: WebSocket, run_id: str):
    ...
    # If Redis configured and client available, use Pub/Sub; else fallback to local queue
    if _REDIS_HOST and aioredis is not None:
        channel_name = f"dm:logs:{run_id}"
        client = aioredis.Redis(host=_REDIS_HOST, port=_REDIS_PORT, decode_responses=True)
        pubsub = client.pubsub()
        await pubsub.subscribe(channel_name)
        # Send recent backlog from Redis list for context
        try:
            recent = await client.lrange(f"dm:logs:{run_id}", 0, 200)
            for line in reversed(recent):  # lrange returns newest first due to LPUSH
                try:
                    await ws.send_text(str(line))
                except Exception:
                    # client closed while sending backlog
                    try:
        ...
        try:
            async for message in pubsub.listen():
                if message and message.get("type") == "message":
                    try:
                        await ws.send_text(str(message.get("data", "")))
                    except Exception:
                        break
...
async def ws_runs(ws: WebSocket):
    ...
    try:
        ...
        pass
    # If Redis configured, stream from Redis pubsub; otherwise from in-process subscribers
    if _REDIS_HOST and aioredis is not None:
        client = aioredis.Redis(host=_REDIS_HOST, port=_REDIS_PORT, decode_responses=True)
        pubsub = client.pubsub()
        await pubsub.subscribe("dm:runs")
        try:
            async for message in pubsub.listen():
                if message and message.get("type") == "message":
                    ...
                    try:
                        ...
                        await ws.send_json(_json.loads(data))
                    except Exception:
            ...
            try:
                await pubsub.unsubscribe("dm:runs")
            except Exception:
                pass
            await client.close()
        return

    # Fallback to local run-state events
